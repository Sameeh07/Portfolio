          ...
          <div className="container mx-auto px-4 py-8 sm:py-12 md:py-20 max-w-4xl">
            ...
            >
              <div className="text-center">
                <p className="font-semibold text-sm sm:text-base">B.E. in Artificial Intelligence &amp; Machine Learning</p>
                ...
                <div className="marquee-track">
                  {[...rowA, ...rowA, ...rowA].map((t, i) => (
                    ...
                    >
                      ...
                      <blockquote className="text-xs sm:text-sm text-gray-300/90 leading-relaxed">"{t.quote}"</blockquote>
                    </figure>
                  ))}
                </div>
              </div>
              ...
              {!sent ? (
                ...
                <div className="rounded-2xl p-6 sm:p-8 flex items-center justify-center border border-emerald-900/40 bg-emerald-900/20">
                  <div className="flex items-center gap-3 text-emerald-300">
                    ...
                    <p className="font-medium text-sm sm:text-base">Your request has been sent! I'll get back to you soon.</p>
                  </div>
                </div>
              )}
  ...
  return (
    <div className="rounded-2xl border border-slate-800 p-4 sm:p-6 bg-slate-900/60 hover:bg-slate-900/80 transition-all shadow-[0_10px_30px_-10px_rgba(0,0,0,0.6)]">
      ...
      <ul className="flex flex-wrap gap-1.5 sm:gap-2">
        {items.map((s) => (
          <li key={s} className="text-xs sm:text-sm rounded-full border border-slate-800 bg-slate-950/40 px-2 sm:px-3 py-1 sm:py-1.5 flex items-center gap-1 sm:gap-1.5 hover:border-fuchsia-600/50 transition-colors">
            <ToolIcon name={s} />
...
function ToolIcon({ name }) {
  ...
  if (/(python|java(script)?|c\+\+|\bc\b|java)/.test(lower)) return <Code2 className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-fuchsia-300" />;
  if (/(react|html|css|frontend|vite)/.test(lower)) return <Globe className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-indigo-300" />;
  if (/(mysql|mongo|postgres|vector|faiss)/.test(lower)) return <Database className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-emerald-300" />;
  if (/(git|github|jenkins|ci|cd|helm|terraform|kubernetes|docker|registry)/.test(lower)) return <GitBranch className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-orange-300" />;
  if (/(aws|gcp|azure|sagemaker|cloud)/.test(lower)) return <Cloud className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-sky-300" />;
  if (/(tensor|torch|keras|scikit|opencv|mlflow|grafana|prometheus|matplotlib)/.test(lower)) return <Boxes className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-rose-300" />;
